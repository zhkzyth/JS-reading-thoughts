            ...
            deferred = {
               ...
               chainCallbacks:function  (callbacks,context,args) {
                  if (callbacks[0]) {
                     ...
                     //   // if (typeof callbacks.shift().apply(context, args).promise === 'function') {
                     //       //do the same things here...
                     //   // } else{
                     //    //but what will happen if it is a normal value
                     //    // return promiseOrValue;
                     //    return newDefer;
                     //   }

                     tmpPromise.then(promise);

                     chainCallbacks(callbacks,context,args);
               ...
               //test whether the function is a promise or value
               //inspired by the when.js
               isPromise:function(){
               }
            };
      ...
      Deferred: function(func) {
         ...
         jQuery.extend(deferred, {
            ...
            always: function() {
               ...
               //fail这里用this作为context是为了保持this仍然指向deferred
               //而callback数组的调用是通过闭包实现引用
            },
            //失败时调用的队列
            ...
            pipe: function(fnDone, fnFail) {
               //jQuery.Deferred会生成一个新的deferred对象
               return jQuery.Deferred(function(newDefer) {
                  ...
                  jQuery.each({
                     ...
                     var fn = data[0],
                        ...
                        returned;
                     if(jQuery.isFunction(fn)) {
                        //给当前的deferred.done或deferred.fail注册一个回调方法
                        //新旧deferred对象之间就是通过下面的handler来实现触发串联的
                        deferred[handler](function() {
                           //把当前fnDone和fnFail塞进当前Deferred对象的callback数组里面
                           //下个newDeferred对象
                           returned = fn.apply(this, arguments);
                           if(returned && jQuery.isFunction(returned.promise)) {
                              //如果当前fn返回一个promise对象，把后续的deferred对象
                              //存入当前的fn对象的回调数组里面，一旦当前deferred对象执行完毕，
                              //马上触发newDeferr对象的action+With方法，让它去跑自己的回调函数数组
                              //然后重复当前这个流程
                              returned.promise().then(newDefer.resolve, newDefer.reject);
                           } else {
                              //否则的话，表示当前的deferred对象状态已经确定，调用下一个newDefer的回调函数数组
                              newDefer[action + "With"](this === deferred ? newDefer : this, [returned]);   //??this ====???
                           }
                        });
                     } else {
                        //如果没传入fnDone或fnFail函数，
                        //利用deferred.done(newDefer.resolve)，传入newDefer.resolve
                        //马上回调newDefer对象自己的callbacks
                        deferred[handler](newDefer[action]);
                     }
                  });
               }).promise();//返回新deferred的promise给外部调用，后续的then、done、reject都是在这个newDeferred对象上的
            },
            // Get a promise for this deferred
            // If obj is provided, the promise aspect is added to the object
            // deferred.promise()返回的是deferred对象的只读属性。
      ...
      if(val > 0) {
         if(extra !== "border") {
            jQuery.each(which, function() {
               ...
               if(extra === "margin") {
                  value += parseFloat(jQuery.css(elem, extra + this)) || 0;
               } else {
