...
jQuery.extend({
   ...
   //参考网址：http://note.sdo.com/u/shuhan/n/6GXE7~jLMWniwE0d400261
...
jQuery.extend({
   ...
   removeData: function( elem, name, pvt /* Internal Use Only */ ) {
      ...
      if ( jQuery.support.deleteExpando || !cache.setInterval ) {    //1.4之后也会删除掉不用的cache[id]项
         delete cache[ id ];
      } else {
         cache[ id ] = null;
      }
      ...
      // false lookups in the cache for entries that no longer exist
         ...
         if ( jQuery.support.deleteExpando ) {     //删除dom节点上对应的数据
...
jQuery.extend({
   ...
   //队列静态方法，get和set都是在这里完成的
   queue: function( elem, type, data ) {
      if ( elem ) {
         type = (type || "fx") + "queue";
         var q = jQuery.data( elem, type, undefined, true );   //队列数据都是写在elem上面的
         ...
         //如果q不为null, 说明缓存上已经存在队列, 但是如果现在被添加的函数是一个数组的话, 以现在的数组来代替原来的队列, 即把原来的队列清空了.
               q = jQuery.data( elem, type, jQuery.makeArray(data), true );   //接受数组参数，都是动画队列么？
            } else {
               q.push( data );
            }
         }
   ...
   dequeue: function( elem, type ) {
      ...
      if ( fn === "inprogress" ) {  //inprogress.....and do it?
         fn = queue.shift();
      }
      ...
      if ( !queue.length ) {
         jQuery.removeData( elem, type + "queue", true );   //true做甚用的？
   ...
   toggleClass: function( value, stateVal ) {
      ...
      return this.each(function() {
         if ( type === "string" ) {
            ...
            this.className = (this.className || value === false) ? "" : (jQuery._data( this, "__className__" ) || "");//运算符的集合性
         }
      });
   },
